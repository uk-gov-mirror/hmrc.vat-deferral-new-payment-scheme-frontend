...
package uk.gov.hmrc.vatdeferralnewpaymentscheme.models
...
import uk.gov.hmrc.vatdeferralnewpaymentschemefrontend.models.EncryptedNuanceData
...
case class EncryptedNuanceDataSpec() extends WordSpec with Matchers {
  ...
  "encryptedNuanceData class" should {
    "allow retrieval of Nuance JSON fields" in {
      ...
      val encryptedNuanceData = EncryptedNuanceData.create(service,
      ...
      val encryptedNuanceData2 = EncryptedNuanceData.create(service,
        HeaderCarrier(sessionId = Some(SessionId("x")), deviceID = Some("y")))
